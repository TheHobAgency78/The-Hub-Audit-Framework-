  ...
  return (
    <div style={{ padding: '20px', background: '#111827', borderRadius: '12px', marginTop: '30px', border: '1px solid #374151', textAlign: 'right', direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        ...
        <button
          onClick={handleSave}
          style={{ padding: '12px 24px', borderRadius: '8px', background: isSaved ? '#10b981' : '#2563eb', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px', transition: 'background 0.2s' }}
        >
...
export const AuditResultView: React.FC = () => {
  ...
  // دالة تشغيل الفحص والاتصال بـ Gemini API بأمان
  const handleStartAudit = async () => {
    setLoading(true);
    setError(null);
    setReport(null);

    // 1. جلب المفتاح السري من الذاكرة المحلية للجهاز
    const apiKey = localStorage.getItem('THE_HUB_GEMINI_KEY');

    // 2. التحقق من وجود المفتاح قبل إرسال الطلب لجوجل
    if (!apiKey) {
    ...
    try {
      // نص البرومبت الأساسي المجهز لوكالة THE HUB (يمكنك تعديله حسب رغبتك)
      ...
      // 3. إرسال الطلب مباشرة إلى مستودع Google AI Studio باستخدام المفتاح المستخرج
      ...
      // 4. استخراج النص البرمجي الراجع وعرضه في الواجهة
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', color: '#fff', fontFamily: 'Arial, sans-serif' }}>
      ...
      {/* قسم عرض الأخطاء في حال عدم وجود المفتاح أو فشل الاتصال */}
      {error && (
      ...
      {/* لوحة العرض والتحكم بالفحص الخوارزمي */}
      <div style={{ background: '#1f2937', padding: '25px', borderRadius: '12px', border: '1px solid #374151', textAlign: 'center' }}>
        ...
        <button
          ...
          disabled={loading}
          style={{ padding: '12px 30px', fontSize: '16px', fontWeight: 'bold', borderRadius: '8px', border: 'none', background: loading ? '#4b5563' : '#4f46e5', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.2s' }}
        >
          {loading ? '⏳ جاري معالجة البيانات وتوليد التقرير...' : '🚀 ابدأ الفحص الخوارزمي الآن'}
        </button>
      </div>

      {/* عرض التقرير الناتج عن الذكاء الاصطناعي */}
      {report && (
      ...
      {/* تضمين مدير المفاتيح السري في أسفل الصفحة لراحة يدك */}
      <ApiKeyManager />
...
export default AuditRe      if (originalStyleSheetDescriptor) {
